...
-   **`main` branch:** The primary integration branch. It represents the latest, stable, development-ready version of the project. **Direct pushes to this branch are forbidden by branch protection rules.**
...
**Goal:** Keep your feature branch updated with the latest changes from the official `main` branch to prevent difficult merges later.

**Scenario:** A teammate has just merged a big feature into the company's `main` branch.

1.  **Fetch the latest history from the `company` remote.** This downloads the new commits but doesn't change your code yet.
    ```bash
    git fetch company
    ```
...
1.  **Open a Pull Request (PR) in VS Code or on GitHub.**
    -   **VS Code:** After pushing your branch, the "GitHub Pull Requests" extension will often show a "Create Pull Request" button.
    -   **On GitHub:** Go to the company repository. You will see a banner for your recently pushed branch with a "Compare & pull request" button.
2.  **Configure the PR:**
    -   **Base Repository:** `Your-Company/CreativeCatalystEngine`
    -   **Base Branch:** `main` (The destination)
    -   **Head Repository:** `Your-Username/creative-catalyst-engine`
    -   **Compare Branch:** `feat/add-image-watermarking` (The source)
...
-   **If they request changes:**
    1.  Make the changes on your local feature branch.
    2.  Commit and `git push origin`. The Pull Request will automatically update.
    3.  Comment on GitHub to let them know you've addressed the feedback.

-   **If you disagree with a suggestion:**
    1.  Reply to the comment on GitHub.
